...
import static proyecto2compi1.sym.*;
...
//TIPO_VARIABLE = "INTEGER"|"BOOLEAN"|"STRING"

//OP RELACIONEALES
//IGUAL, MAYOR Y MENOR  QUE SE DECLARARON COMO ABRE Y CIERRA POR LAS ETIQUETAS PERO RECONOCERLOS APARTE
//MENOR_QUE = "<"
//MAYOR_QUE = ">"
MAYOR_IGUAL = ">="
MENOR_IGUAL = "<="
...
%{
	...
	private void error(String error, int linea, int columna) {
		...
		this.panel.agregarErrorLexico(linea,columna,mensaje,indexTab);
		this.panel.agregarError(linea,columna,mensaje,,indexTab);
	}

	public Lexer (java.io.Reader entrada, JTextArea textArea, JFramePrincipal panel, int indexTab){
...
<YYINITIAL> {
	...
	{ABRE_EMBE}
		{System.out.printf(" ABRE EMBEBIDO: %s\n", yytext()); return symbol(ABRE_EMBE, yytext());}
	{CIERRA_EMBE}
		{System.out.printf(" CIERRA EMBEBIDO: %s\n", yytext()); return symbol(CIERRA_EMBE, yytext());}
	{HTML}
	...
	{VAR}
		{System.out.printf(" VARIABLE: %s\n", yytext()); return symbol(VAR, yytext());}
	{INTEGER}
		{System.out.printf(" VARIABLE: %s\n", yytext()); return symbol(INTEGER, yytext());}
	{BOOLEAN}
		{System.out.printf(" VARIABLE: %s\n", yytext()); return symbol(BOOLEAN, yytext());}
	{STRING}
		{System.out.printf(" VARIABLE: %s\n", yytext()); return symbol(STRING, yytext());}
	//{TIPO_VARIABLE}
	//	{System.out.printf(" TIPO_VARIABLE: %s\n", yytext()); return symbol(TIPO_VARIABLE, yytext());}
	{TRUE}
		{System.out.printf(" PALABRA RESERVADA: %s\n", yytext()); return symbol(TRUE, yytext());}
	{FALSE}
	...
	{THEN}
		{System.out.printf(" PALABRA RESERVADA: %s\n", yytext()); return symbol(THEN, yytext());}
	{WHILE}
		{System.out.printf(" PALABRA RESERVADA: %s\n", yytext()); return symbol(WHILE, yytext());}
	{FOR}
		{System.out.printf(" PALABRA RESERVADA: %s\n", yytext()); return symbol(FOR, yytext());}
	{TO}
		{System.out.printf(" PALABRA RESERVADA: %s\n", yytext()); return symbol(TO, yytext());}
	{PRINT}
		{System.out.printf(" PALABRA RESERVADA: %s\n", yytext()); return symbol(PRINT, yytext());}
	{BEGIN}
		{System.out.printf(" PALABRA RESERVADA: %s\n", yytext()); return symbol(BEGIN, yytext());}
	{END}
		{System.out.printf(" PALABRA RESERVADA: %s\n", yytext()); return symbol(END, yytext());}

	{MAYOR_IGUAL}
		{System.out.printf(" MAYOR IGUAL: %s\n", yytext()); return symbol(MAYOR_IGUAL, yytext());}
	{MENOR_IGUAL}
		{System.out.printf(" MENOR IGUAL: %s\n", yytext()); return symbol(MENOR_IGUAL, yytext());}
	{DIFERENTE}
		{System.out.printf(" DIEFERENTE: %s\n", yytext()); return symbol(DIFERENTE, yytext());}
	...
	{DOS_PT}
		{System.out.printf(" DOS PUNTOS: %s\n", yytext()); return symbol(DOS_PT, yytext());}
	{DOS_PT_IGUAL}
		{System.out.printf(" DOS PUNTOS IGUAL: %s\n", yytext()); return symbol(DOS_PT_IGUAL, yytext());}
	{PT_COMA}
	...
	//{APOSTROFE}
	//	{System.out.printf(" APOSTROFE: %s\n", yytext()); return symbol(APOSTROFE, yytext());}
	

	//{TIPO_CARACTER}
	//	{System.out.printf(" PARAMETRO: %s\n", yytext()); return symbol(TIPO_CARACTER, yytext());}
	//{CODIGO_COLOR}
	...
	{PARAMETRO_TEXTO}
		{System.out.printf(" TEXTO ENTRE COMILLAS: %s\n", yytext()); return symbol(PARAMETRO_TEXTO, yytext());}
	{IDENTIFICADOR}
