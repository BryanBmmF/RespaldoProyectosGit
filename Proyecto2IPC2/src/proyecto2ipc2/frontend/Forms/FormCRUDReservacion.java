 ...
 * @author bryan
 */
public class FormCRUDReservacion extends javax.swing.JInternalFrame {

    /**
     * Creates new form FormCRUDReservacion
     */
    ...
    public FormCRUDReservacion(JFramePrincipal ventana) {
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("Gestion de Reservaciones");
        ...
        if (tbHabitaciones.getColumnModel().getColumnCount() > 0) {
            tbHabitaciones.getColumnModel().getColumn(4).setHeaderValue("Estado");
        }
        ...
        // TODO add your handling code here:
            if (txtNombreHuesped.getText().isEmpty()||txtFechaIngreso.getText().isEmpty()||txtFechaSalida.getText().isEmpty()
                    ...
                    JOptionPane.showMessageDialog(null, CAMPOS_OBLIGATORIOS, ERROR_CREACION, JOptionPane.ERROR_MESSAGE);
                        ...
                        if (txtNoReservacion.getText().isEmpty()) {
                                ...
                                if (opcion==0) {
                                        ...
                                        //dias entre las fechas
                                        Period dif= Period.between(fechaIngreso, fechaRetiro);
                                        System.out.println(""+dif.getDays());
                                               
                                        cliente = new Cliente((String)comboNits.getSelectedItem(),txtNombreHuesped.getText());
                                        ...
                                        reservacion= new Reservacion(cliente,habitacion,fechaIngreso, fechaRetiro,"NO_ACTIVA", hotel);
                                        controladorReserv= new ControladorReservacion(reservacion, this);
                                        controladorReserv.verificarRegistroReservacion();
                                ...
                                JOptionPane.showMessageDialog(null, RESERVACION_SELECCIONADA, ERROR_CREACION, JOptionPane.ERROR_MESSAGE);   
                            }
    ...
    private void tbHabitacionesMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbHabitacionesMouseClicked
        // TODO add your handling code here:
            ...
            if(tbHabitaciones.getSelectedRow()!=-1){//si se ha seleccionado algo en la tabla se rellenan los campo
                    txtNumeroHabitacion.setText(model.getValueAt(tbHabitaciones.getSelectedRow(), 0).toString());
            ...
            if (txtNombreHuesped.getText().isEmpty()||txtFechaIngreso.getText().isEmpty()||txtFechaSalida.getText().isEmpty()
                    ...
                    JOptionPane.showMessageDialog(null,CAMPOS_OBLIGATORIOS, MODIFICACION, JOptionPane.ERROR_MESSAGE);
                        ...
                        if (opcion==0) {
                                ...
                                reservacion= new Reservacion(cliente,habitacion,fechaIngreso, fechaRetiro,"NO_ACTIVA", hotel);
                                controladorReserv= new ControladorReservacion(reservacion, this);
                                controladorReserv.verificarActualizacionReservacion();
    ...
    private void btnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarActionPerformed
        // TODO add your handling code here:
            if (txtNoReservacion.getText().isEmpty()) {
                JOptionPane.showMessageDialog(null, ELIMINAR_RESERVACION_SELECCIONADA, ELIMINACION, JOptionPane.ERROR_MESSAGE);
            } else {    
                        int opcion= JOptionPane.showConfirmDialog(this, ELIMINAR_DATOS_RESERVACION+idReservacion+"?");
                        if (opcion==0) {
                                habitacion= new Habitacion(Integer.parseInt(txtNumeroHabitacion.getText()));
                                hotel = new Hotel(Integer.parseInt(txtIdHotel.getText()));
                                reservacion= new Reservacion(Integer.parseInt(txtNoReservacion.getText()),habitacion,hotel);
                                controladorReserv= new ControladorReservacion(reservacion, this);
                                controladorReserv.eliminarReservacion();
                        }
                        if(opcion==1){
                                limpiarCampos();
                                deshabilitarBotones();
    ...
    public void cargarTablaReservaciones(){
            controladorReserv= new ControladorReservacion();
            List<Reservacion> lista= controladorReserv.recuperarReservaciones(Integer.parseInt(txtIdHotel.getText()));
    ...
    public void agregarReservacionEnTabla(Reservacion reserv){
            DefaultTableModel model= (DefaultTableModel)this.tbReservaciones.getModel();
            String fechaIngreso= reserv.getFechaIngreso().format(DateTimeFormatter.ofPattern("dd/MM/yyyy"));
            String fechaRetiro= reserv.getFechaRetiro().format(DateTimeFormatter.ofPattern("dd/MM/yyyy"));
            model.addRow(new Object[]{reserv.getNumero(),reserv.getCliente().getNombre(),
                    reserv.getCliente().getNit(),reserv.getHabitacion().getNumero(),reserv.getHabitacion().getPrecio(),
                    fechaIngreso,fechaRetiro});
    ...
    public void rellenarCampos(Reservacion reserv){
            
            txtNoReservacion.setText(String.valueOf(reserv.getNumero()));
            txtNombreHuesped.setText(reserv.getCliente().getNombre());
            comboNits.setSelectedItem(reserv.getCliente().getNit());
            txtNumeroHabitacion.setText(String.valueOf(reserv.getHabitacion().getNumero()));
            txtPrecio.setText(String.valueOf(reserv.getHabitacion().getPrecio()));
            txtFechaIngreso.setText(String.valueOf(reserv.getFechaIngreso()));
            txtFechaSalida.setText(String.valueOf(reserv.getFechaRetiro()));
    ...
    public void deshabilitarBotonesParaClientes(){
    ...
    public void cargarTablasHabitacion(){
     ...
     public int retornarIdReservacionTemporal(){
